...
function getCircleCircumference(radius) {
  const pi = Math.PI;
  const circumference = radius * pi * 2;
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  const deltaY2 = deltaY ** 2;
  const distance = Math.sqrt(deltaX2 + deltaY2);
  return distance;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const x1Square = x1 ** 2;
  const y1Square = y1 ** 2;
  const firstVectorLength = Math.sqrt(x1Square + y1Square);

  const x2Square = x2 ** 2;
  const y2Square = y2 ** 2;
  const secondVectorLength = Math.sqrt(x2Square + y2Square);

  const lengthsProduct = firstVectorLength * secondVectorLength;
  const angleCosine = dotProduct / lengthsProduct;
...
function getParallelepipedDiagonal(a, b, c) {
  ...
  const bSquare = b ** 2;
  const cSquare = c ** 2;
  const squaresSum = aSquare + bSquare + cSquare;
  const diagonal = Math.sqrt(squaresSum);
  return diagonal;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  const number = num / multiplier;
  const numberRounded = Math.round(number);
  const answer = numberRounded * multiplier;
  return answer;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const sum = x1 + x2 + x3;
  const fixedSum = sum.toFixed(10);
  return Number(fixedSum);
}
...
function getCountOfOddNumbers(number) {
  ...
  const half = absoluteNumber / 2;
  const count = Math.ceil(half);
